...
public sealed class PhotosModel : PageModel
{
    private readonly AppDbContext _db;
    private readonly CarPhotoStorage _photos;

    public PhotosModel(AppDbContext db, CarPhotoStorage photos)
    {
        _db = db;
        _photos = photos;
    }
    ...
    {
        var car = await LoadOwnedListingAsync(track: false);
        if (car is null)
            return NotFound();
    ...
    {
        var car = await LoadOwnedListingAsync(track: true);
        if (car is null)
            return NotFound();

        if (car.IsSold)
        {
            TempData["StatusMessage"] = "Нельзя добавлять фото к проданному объявлению.";
            return RedirectToPage("/Cars/Details", new { id = car.Id });
        }
        ...
        if (files.Count > CarPhotoStorage.MaxFilesPerUpload)
        {
            ModelState.AddModelError(nameof(NewPhotos),
                $"За раз можно загрузить не больше {CarPhotoStorage.MaxFilesPerUpload} файлов.");
            return Page();
        }
        ...
        {
            ModelState.AddModelError(nameof(NewPhotos),
                $"У объявления может быть не больше {CarPhotoStorage.MaxPhotosPerCar} фото (сейчас {currentCount}).");
            return Page();
        }
    ...
    public async Task<IActionResult> OnPostDeleteAsync(int photoId)
    {
        ...
        if (!photo.Car.IsCustomerListing() || !photo.Car.IsOwnedBy(User))
            return NotFound();

        if (photo.Car.IsSold)
        {
            TempData["StatusMessage"] = "Нельзя удалять фото у проданного объявления.";
            return RedirectToPage("/Cars/Details", new { id = photo.Car.Id });
        }
    ...
    private async Task<Car?> LoadOwnedListingAsync(bool track)
    {
        var query = track ? _db.Cars : _db.Cars.AsNoTracking();
        var car = await query.FirstOrDefaultAsync(x => x.Id == Id);
        if (car is null)
            return null;

        if (!car.IsCustomerListing() || !car.IsOwnedBy(User))
            return null;
