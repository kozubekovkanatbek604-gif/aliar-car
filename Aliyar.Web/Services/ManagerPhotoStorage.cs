...
public sealed class ManagerPhotoStorage
{
    ...
    public ManagerPhotoStorage(IWebHostEnvironment env)
    {
        _env = env;
    }
    ...
    public async Task<string> SaveAsync(string userId, IFormFile file, CancellationToken ct = default)
    {
        ...
        var uploadsDir = Path.Combine(_env.WebRootPath, "uploads", "managers");
        Directory.CreateDirectory(uploadsDir);

        var fileName = $"{userId}_{Guid.NewGuid():N}{extension}";
        var physicalPath = Path.Combine(uploadsDir, fileName);
        var relativePath = $"uploads/managers/{fileName}";
